...
- [x] Memory: `sink.Flusher` at 8192 for combat / actions / intervals;
      delete the `match_id` on decode error after a flush
- [x] Perf: consume unused entity bitstreams without storing trees
      ...
      **Superseded:** decode is now manta; unused classes stay in its
      entity map. Extract RAM is still O(batch) via mid-decode flush.
