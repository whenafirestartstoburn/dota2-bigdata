...
| Live | `poll_realtime_stats` | live | Для матчей в `live` с `server_steam_id` вызывается GetRealtimeStats. Upsert в Postgres `matches`, `match_players`. Тики — в ClickHouse `live_match_ticks` и `live_player_ticks`. Статус матча не меняет. |
