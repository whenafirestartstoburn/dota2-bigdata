...
describe('asSteamId64', () => {
	test('keeps a 17-digit server id as decimal text', () => {
		expect(asSteamId64('90292220717473792')).toBe('90292220717473792')
		expect(asSteamId64(90292220717473792n)).toBe('90292220717473792')
	})

	test('accepts safe integers and rejects already-rounded numbers', () => {
		expect(asSteamId64(1)).toBe('1')
		expect(asSteamId64(90292220717473792)).toBeNull()
		expect(asSteamId64(-1)).toBeNull()
	})
})
...
describe('asItemId', () => {
	test('maps Valve empty -1 to 0 and keeps missing as null', () => {
		expect(asItemId(-1)).toBe(0)
		expect(asItemId(63)).toBe(63)
		expect(asItemId(null)).toBeNull()
	})
})
